...
struct Animal {
    ...
    // 실패가능 생성자
    init?(species: String) { //생성자에 ? 를 붙힌개념
        if species.isEmpty {
...
/*
 ...
 (동일단계 - 델리게이트 어크로스)
 - 실패가능생성자에서 실패불가능생성자 호출  (호출/위임) (OK)
 - 실패불가능생성자에서 실패가능생성자 호출 (호출/위임) (X)
 
 (상속관계 - 델리게이트 업)
 - (상위)실패가능생성자  <=== (하위)실패불가능생성자  (호출/위임)  (X)
 - (상위)실패불가능생성자 <=== (하위)실패가능생성자   (호출/위임)  (OK)
...
결국 상속관계에서는 꼭 슈퍼클래스가 실패가능생성자야 된다는것.   
슈퍼클래스가 실패가능생성자면 하위클래스는 실패가능이던~ 실패불가능이던 다 된다.