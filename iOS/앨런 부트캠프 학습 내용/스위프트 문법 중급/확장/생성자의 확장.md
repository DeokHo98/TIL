...
//즉 클래스 = 편의생성자만 구현이 가능하다.  구조체 = 지정생성자 형태로도 구현이 가능하다.
//예외적으로 구조체에서 기본생성자와 멤버와이즈생성자가 제공이 된 상황 = 모든저장속성에 기본값제공 + 생성자 구현 안함
//일 경우에는 확장에서 생성자를 구현해도 본체에서는 여전히 기본생성자/멤버와이즈 생성자가 제공되고 호출하는 방식으로 구현하는것도 가능하다
...
// UIColor 기본 생성자
var color = UIColor(red: 0.3, green: 0.5, blue: 0.4, alpha: 1) //색깔을 생성하는 생성자.




extension UIColor {      //불편한점이있어서 익스텐션 + 편의생성자 조합으로 새로운 생성자루 구현해보자
    
    convenience init(color: CGFloat) {   // Float = 더블같은 실수형 타입
        self.init(red: color/255, green: color/255, blue: color/255, alpha: 1)//지정생성자를 호출해야함
    } //색이 0부터 255까지 있음 가짓수가
